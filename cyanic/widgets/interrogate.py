...
class InterrogateWidget(QWidget):
    def __init__(self, settings_controller:SettingsController, api:SDAPI, prompt_widget: PromptWidget, image_in:ImageInWidget, mode:str, size_dict={"x":0,"y":0,"w":0,"h":0}):
        super().__init__()
        ...
        if self.debug:
            ...
            self.debug_data.setPlaceholderText('JSON data used to interrogate the image will be shown here')
    ...
    def interrogate(self):
        ...
        try:
            ...
            self.interrogate_btn.setText('Interrogate') # Want the UI to look right, even if we have an exception
            self.update()
        ...
        if self.results is not None:
            ...
            if "caption" in self.results:
                self.prompt_widget.prompt_text_edit.setPlainText(self.results["caption"])
